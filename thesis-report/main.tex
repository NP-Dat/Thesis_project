...
For the frontend, we decided to go with Next.js~16 using the App Router and React~19, keeping everything strictly typed with TypeScript, and we rely on TanStack Query to manage caching, background refetching, and cache invalidation whenever a mutation happens. All of our forms are hooked up to react-hook-form and validated using Zod schemas. For the data visuals, we chose Recharts to render the charts, and we grab our icons from lucide-react.

One big architectural shortcut we took is that the application is built strictly for desktop screens. Since the primary target environments are HR office terminals and factory kiosk stations, we completely skipped implementing mobile breakpoints to save time.

\subsubsection{Route Structure and Navigation}

The entire frontend architecture maps out to 13 specific routes, which we split cleanly into three buckets: public authentication routes, employee portal pages protected by the \texttt{role = ``employee''} requirement, and admin portal screens locked behind the \texttt{role = ``admin''} permission.
...
To enforce this setup, each portal layout wraps its children inside a custom \texttt{RouteGuard} component. This component intercepts the navigation to verify the user's current authentication status and specific role. If someone unauthenticated tries to sneak in, it boots them back to \texttt{/login}, and if an authenticated user tries to access the wrong portal, it automatically redirects them right back to their own designated dashboard.

\subsubsection{Design System}

Our visual identity uses a warm, paper-like color palette that we encoded entirely as Tailwind~v4 CSS custom properties. We made sure every single color used across the application maps back directly to a named design token, so you cannot find any random, ad-hoc hex values thrown around the conponent code. You can check out Table~\ref{tab:design-tokens} to see how the primary tokens are laid out.
...
For the typography, we went with a Georgia serif typeface for the headlines to give the app a more formal, document-like vibe, but kept the body text and UI elements clean using the Inter sans-serif family. We also established four distinct risk-level colors, which makes it super easy to get an instant visual classification whenever they pop up on charts, badges, and status indicators throughout the interface.

\subsubsection{Data Flow Pattern}

We handled all server communication by sticking to a strict, four-layer architecture pattern to keep things uniform across the codebase.

First, the actual Page component sits at the top, rendering the UI and pulling in a custom TanStack Query hook. That custom hook (\texttt{useXxx}) acts as the second layer, wrapping around \texttt{useQuery} or \texttt{useMutation} to manage things like cache keys, enabled conditions, and invalidation rules. Below that, the third layer is the API function (\texttt{lib/api/}), which triggers a strongly typed \texttt{apiFetch()} call pointing to the right endpoint path and arguments. Finally, everything funnels into the base Fetch client (\texttt{lib/api/client.ts}). This low-level client automatically tacks on the base URL, injects the user's JWT directly from \texttt{localStorage}, unpacks the standard \texttt{{success, data}} response envelope, and throws a typed \texttt{ApiError} if anything breaks. If it intercepts a 401 Unauthorized status, it instantly wipes the bad token and boots the user back to the login page.

For anything altering data, we just hook up mutations using \texttt{useMutation} and trigger cache invalidation the second they succeed. This keeps the UI completely fresh and up to date without forcing annoying full-page reloads.
...
The backend codebase mirrors the exact folder layout we established back in Section~3.3.1, drawing sharp lines between configuration, middleware, models, services, controllers, routes, validators, and utility scripts. To stop configuration bugs before they happen, all environment variables run through a Zod validation schema inside \texttt{config/env.js} the moment the application kicks off. If a required variable is missing or formatted wrong, the process immediately crashes with a clear error message, ensuring the server never boots up in a broken, half-configured state.

When the server spins up, it connects to the databases in a strict chain reaction. First, \texttt{server.js} pings MySQL, jumps over to connect to MongoDB next, and only opens up the HTTP listener once both databases are happy. If any link in this chain breaks, the whole startup sequence aborts on the spot. We also hooked up SIGINT and SIGTERM event listeners to manage graceful shutdowns, meaning the app will systematically stop the HTTP server first before safely closing down both database connections.

\subsubsection{Authentication Flow Implementation}

Whenever a new user registers, the backend handles their account creation and spits out a synthetic HR profile simultaneously inside a single MySQL transaction block.

First, Zod parses and validates the request body to check the email, password, name, gender, and date of birth. Then, bcrypt hashes the password using our configured salt rounds before the system inserts a new row into the \texttt{users} table. Right after that, the code cooks up a simulated HR profile by randomly picking a department and generating mock metrics for the user's designation (1-5), resource allocation (1-10), shift type, company type, WFH availability, and a fake joining date. This profile gets saved directly into the \texttt{hr\_profiles} table. Finally, the app signs a JWT containing the new user's ID and role, sending that token and the user data back to the client.

Wrapping this entire sequence in a database transaction is super important here, because if the HR profile generation throws an error, the database rolls back completely and prevents an orphaned user record from clogging up our tables. Logging in works a lot more simply, just checking incoming credentials against the stored bcrypt hash and handing out a fresh JWT with an expiration that defaults to 7~days.


\subsubsection{Quiz Scoring Implementation}

The backend relies on the quiz submission handler inside \texttt{quiz.service.js} to run the full pipeline we laid out back in Section~3.3.3. First, it goes through basic answer validation. The system expects exactly 13 answers, checks that all the question IDs are actually valid, flags any duplicate IDs, and forces the answer values to fit into the specific set $\{0, 25, 50, 75, 100\}$. Then there is Question~10, which asks, ``Do you have enough energy for family and friends during leisure time?''. This one requires reverse scoring, so we use \texttt{adjustedValue = 100 - rawValue}, keeping both the original and adjusted values inside the enriched response object. For data safety, we chose a MongoDB-first persistence approach. The raw submission hits MongoDB before we even try making the AI call, so we do not lose user answers if things crash later. If the next steps fail, the system just cleans up that orphan document. Finally, saving the assessment results and checking the alert rules happens inside a single, transactional MySQL write. Once that goes through, the MongoDB document gets back-linked with the new MySQL assessment ID.

\subsubsection{Dashboard Aggregation}

To load the admin dashboard, a single API call figures out all the company-wide and department-level stats. We used a correlated subquery to pull the most recent \texttt{assessment\_results.id} for each employee so we only look at their latest data. JavaScript then takes this result set and aggregates it to build out a few things. It calculates the company overview, which covers total employees, total assessments, average burn rate, and how the risk numbers are distributed. It also handles the department breakdown by looking at employee counts, average burn rates, high-risk numbers, and risk distributions. On top of that, it generates department KPIs by averaging out attendance, productivity, and quality scores, and it grabs the 10 most recent alert records. If an admin uses the department drill-down endpoint, they get server-side pagination, sorting, and filtering by things like risk level, shift type, or designation.

\subsubsection{Alert System Implementation}

Right after an assessment finishes, the alert service runs the two rules we talked about in Section~3.3.4. The system runs both the threshold check and the trend spike comparison right inside that same MySQL transaction that handles the assessment result. For the trend spike threshold, the default is 0.15, but you can change this whenever you want by tweaking the \texttt{ALERT\_TREND\_SPIKE\_DELTA} environment variable.

\subsubsection{API Endpoint Summary}

We put together Table~\ref{tab:api-summary} to give a quick summary of all the RESTful endpoints the backend exposes. If you need the full API reference with all the specific request and response schemas, it can be find in Appendix~A.
...
We split our preprocessed dataset using a standard 80/20 train/test split, leaving us with around 22{,}000 rows to work with after stripping out any null targets. To see how well our setups actually performed, we tested them on the held-out test set. You can see the full breakdown of the evaluation results in Table~\ref{tab:model-comparison}.
...
Looking at the numbers, the Random Forest model hit an $R^{2}$ of 0.90, which basically means it manages to explain 90\% of the variance in the burn rate data. It also got an MAE of 0.048. In plain terms, that tells us the predictions usually miss the real values by less than 5 percentage points when you map it on that 0--1 scale. It clearly beat Linear Regression across all three metrics, giving us a solid 3.6 percentage-point jump in $R^{2}$.
...
When looking at the feature importance analysis shown in Figure~\ref{fig:feature-importance}, the mental fatigue score completely dominates the predictions. It carries about 85\% of the model's total predictive power. This makes total sense if you think about how burnout works in the real world. A worker who is completely exhausted psychologically is going to face a high risk of burnout, and things like how senior they are or how heavy their workload is won't change that much.
...
For the prediction endpoint, we used FastAPI and hooked it up with Pydantic models to handle input validation. It enforces some strict boundaries on the incoming data: designation has to be between 1--5, resource allocation ranges from 1--10, and the mental fatigue score goes from 0--10. To keep things fast, a lifespan handler loads the machine learning model once right at startup and caches it directly in memory. When a prediction request hits the server, the code sets up the feature array, fires off \texttt{model.predict()}, clamps the output so it stays within the valid [0,~1] range, and sends back the burn rate inside our standard envelope format. The best part is that response times are incredibly quick, consistently clocking in under 10~milliseconds.

\subsubsection{Integration with Express Backend}

Over on the Express side, \texttt{ai.service.js} handles things using a dual-mode setup that depends entirely on the \texttt{AI\_USE\_MOCK} environment variable. When you turn this variable to \texttt{false}, the service throws an HTTP POST request at the Python microservice with those three features and grabs \texttt{response.data.predictedBurnRate}. We also wrapped the request in an \texttt{AbortController} using a configurable timeout variable called \texttt{AI\_SERVICE\_TIMEOUT\_MS}. If the connection times out or the server returns a non-2xx status, the code intercepts it, formats it as an \texttt{ApiError.internal()}, and sends a generic 500 error up to the frontend.

Now, if you flip that variable to \texttt{true}, the backend skips the Python call entirely and uses this mock formula locally:
\begin{equation}
...
\noindent here, \textit{mfs} represents the mental fatigue score, \textit{d} stands for designation, \textit{ra} maps to resource allocation, and \textit{noise} injects a bit of uniform random variance between $[\!-0.025, +0.025]$. It makes testing a lot easier since jumping between real AI predictions and the mock math requires zero code refactoring, you just tweak the environment variable and bounce the server.

\subsection{Frontend Implementation}

\subsubsection{Authentication Pages}

We went with a clean split-screen look for both the login and registration pages. The left side holds a brand illustration paired with a serif headline over a nice parchment background, while the actual form sits on an ivory surface on the right side. To keep the forms responsive and secure, we managed them with react-hook-form and backed them up with Zod validation schemas, which catches any input errors on the client side before anything even gets submitted.
...
The employee portal is built around a sidebar navigation menu that gives users quick access to four primary pages: Dashboard, Assessment, Results, and History.

Employee Dashboard (\texttt{/dashboard}). Think of this as the central hub for the user. It starts by displaying the employee's basic profile info. Right next to that, we built a custom SVG semicircle gauge with a moving needle and color-coded risk labels to visually represent their current burn rate. There is also a bar chart that compares personal versus work-related burnout scores on a 0--100 scale, alongside a line chart tracking how their burn rate has moved over time. We overlaid this line chart with reference lines at the 0.35, 0.55, and 0.80 risk thresholds to make reading the trends obvious. If someone hits a high or critical risk level, the dashboard dynamically renders a special ``Get Help'' panel packed with mental health resources. On the flip side, if a new user logs in and hasn't filled out any assessments yet, the app displays a clean empty state with a big call-to-action button prompting them to take their very first quiz.
...
CBI Assessment (\texttt{/assessment}). A two-step wizard
...
Results (\texttt{/results}). Displays the outcome of the most
...
History (\texttt{/history}). A paginated table listing all past
...
Admin Dashboard (\texttt{/admin/dashboard}). The command centre
...
Department Detail (\texttt{/admin/departments/[id]}). A two-tab
...
Employee Detail (\texttt{/admin/employees/[id]}). A deep-dive
...
Alerts (\texttt{/admin/alerts}). This page functions like a filterable inbox, complete with dropdowns to sort by read status and alert type. The main table displays the alert type badge, the department, the actual alert message, timestamps, and status indicators. Admins can click individual Mark read'' buttons or use a global Mark all read'' action. To keep the UI snappy, all these mutations use TanStack Query cache invalidation for instant state updates without reloading.
...
Resource Management (\texttt{/admin/resources}). This is where full CRUD operations happen for the help resources. The main table lists out every available resource with its title, description, URL, an active status toggle, and a badge for the minimum risk level required to see it. Creating or updating a resource opens up a modal form backed by Zod validation, while trying to delete something triggers a safety confirmation prompt. Standard toast notifications pop up to confirm whenever an operation goes through successfully.
...
Our frontend architecture breaks down into 15 reusable UI primitives stored inside \texttt{components/ui/} alongside 12 distinct chart components located in \texttt{components/charts/}. A few custom pieces are worth highlighting here. First is the \textbf{BurnRateGauge}, which uses a custom SVG semicircle to draw a color-gradient arc shifting from green to amber and finally to red. We added an animated needle that points directly to the current burn rate value, backed up by a text label underneath showing the specific risk level. Next, the \textbf{DepartmentHeatmap} renders out a grid of tiles. Each department gets its own clickable card displaying the name and total employee count, while the background color is dynamically interpolated from our risk scale based on the department's average burn rate. Finally, we built the \textbf{KpiBurnRateComparisonChart} using a Recharts composed layout. It utilizes dual Y-axes to plot the composite KPI as a bar chart against the average burn rate as a line chart over monthly intervals, making any inverse correlations between performance and burnout completely obvious.

\subsubsection{State Management with TanStack Query}

We pushed all of our server state management into TanStack Query, which takes care of automatic caching, background data refetching, and cache invalidation right out of the box. Instead of calling the library directly everywhere, we wrapped everything in custom hooks inside \texttt{lib/hooks/}, utilizing \texttt{useQuery} for read-heavy operations and \texttt{useMutation} for writing data. Whenever a mutation succeeds, it triggers an automatic invalidation of the related query keys. This keeps the UI completely synchronized with whatever is on the database without forcing us to trigger manual refetch calls. For the user experience side of data fetching, we used pulsing sand-colored skeleton blocks to cover loading states, while errors pop up as inline messages inside crimson-tinted cards. If a query returns nothing, the application renders a clean \texttt{EmptyState} component featuring an icon, descriptive text, and an actionable button to guide the user next.
...
The Random Forest model hits an $R^{2}$ of 0.90, an MAE of 0.048, and an RMSE of 0.061 when tested against our held-out test set, which we logged in Table~\ref{tab:model-comparison}. In plain terms, these numbers prove the model explains 90\% of the variance in burn rate, usually staying accurate to within 5 percentage points on that standard 0--1 scale. If you compare it to Linear Regression, we see a 3.6 percentage-point jump in $R^{2}$. This clearly shows that the Random Forest algorithm does a great job capturing the non-linear interactions happening between our three features, things a basic linear model completely misses unless you manually engineer features yourself.When you look at the feature importance analysis, the mental fatigue score basically carries the whole model, accounting for roughly 85\% of its predictive power. Resource allocation takes up about 10\%, while designation sits at a tiny 5\%. This breakdown matches up perfectly with actual domain knowledge. An employee's psychological state is always going to be the most direct flag for burnout risk, while how heavy their workload is or how senior they are only play secondary roles. Plus, having mental fatigue dominate like this validates our whole system design, since we placed the CBI quiz, which is where that fatigue score comes from, right at the center of the user workflow.Looking at the residual analysis in Figure~\ref{fig:residual-analysis}, the prediction errors seem randomly scattered around zero without any weird, systematic patterns. This means the model isn't leaning into habitual over- or under-prediction as the burn rate changes. The residuals actually follow a pretty normal distribution centered right at zero, which is exactly what you want to see in a solid regression model.We also compared our results against public Kaggle notebooks that trained on this exact same dataset but used every single available feature. Even though we stripped our model down to just three features, our $R^{2}$ of 0.90 sits right comfortably in their reported 0.90--0.93 range for Random Forests. It shows that our three chosen features hold almost the entire predictive signal, meaning extra data columns like Gender, Company Type, or WFH Setup don't actually add much value when predicting burn rates.
...
All 23 test cases passed successfully, proving that our core workflows run exactly the way we designed them. We made sure the testing phase covered everything, including authentication, quiz scoring, AI integration, dashboard aggregation, alert generation, resource CRUD operations, and basic access control.

\subsection{System Feature Evaluation}

We set up Table~\ref{tab:objective-eval} to evaluate each project objective we originally laid out in Section~1.4 against the actual system features we ended up implementing.
...
All six of our main objectives are officially locked down. On the user side, the employee portal gives workers a solid way to track their own mental health using clean gauge visualisations, historical trend charts, and tailored support resources. For the management side, the admin portal equips HR with department-level heatmaps, risk distribution analytics, KPI correlation views, and a fully operational alert inbox. Plus, the whole thing runs on a microservice architecture that cleanly cuts the ML logic away from our main web backend.
...
Our Random Forest model hit an $R^{2}$ of 0.90 alongside an MAE of 0.048. This proves that you only need three features—designation, resource allocation, and the mental fatigue score—to build an effective burn rate predictor. What is great is that the model's performance holds its own against published setups using the complete feature set, meaning our three chosen columns capture almost the entire signal hidden in the data.

\textbf{RQ2: Can a web-based system provide actionable burnout insights to
both employees and HR administrators?}

Because the employee portal rolls together personalized burn rate gauges, trend lines, section breakdowns, and contextual help, workers can stay ahead of their mental health proactively. At the same time, the admin portal hands HR managers actionable visibility into organizational burnout patterns through department heatmaps, risk distribution charts, and automated alerts. Our functional testing runs back this all up, confirming that data moves correctly from the moment a quiz is submitted, through the AI prediction step, and straight onto the dashboards.
...
Going with a microservice architecture paid off in three big ways. First, we got technology heterogeneity—meaning we could use Python's scikit-learn ecosystem where it excels for ML, while keeping JavaScript's Express ecosystem for the web logic. Second, it allows for completely independent development since the AI service can be retrained and redeployed without touching or bouncing the main backend. Third, the mock/real switch (\texttt{AI\_USE\_MOCK}) lets us develop the app with full functionality even without running the Python service locally. The only real downside here is the extra operational complexity, since you have to keep two servers up and running, plus handle network timeouts and error propagation if they lose connection with each other.

\subsection{Comparison with Related Work}

Looking at Table~\ref{tab:related-comparison}, this system stands out because it is the only solution in the reviewed literature that actually glues everything together: a trained ML model, a live web app, analytics for the whole organization, and a scientifically validated burnout scale like the CBI. Other papers only solve pieces of the puzzle. For instance, Pradhan and Jena~\cite{pradhan2019} built a solid machine learning framework to classify burnout, but they stopped there—no web deployment or organizational dashboards for managers. On the commercial side, tools like Culture Amp~\cite{cultureamp2024} give companies nice web dashboards, but their backend relies on standard engagement surveys instead of predictive ML models. Even open-source attempts like the Kaggle community notebooks~\cite{kaggle2021} show how to train models on this exact same dataset, yet they leave the code sitting in notebooks without connecting it to a real, working application. This thesis plugs that gap. By combining psychometric testing, machine learning, and multi-level analytics into one single platform, the system serves both the individual employee who needs to check on their well-being and the executive making company-wide decisions.
...
This thesis covered the design, setup, and testing of an intelligent web framework meant to spot employee burnout early in industrial settings. What makes this system work is how it connects three different parts: the Copenhagen Burnout Inventory (CBI) to get reliable human data, a Random Forest regression model to predict risk levels, and a full-stack web application with distinct portals tailored for employees and administrators.

The machine learning side turned out surprisingly efficient. By training a Random Forest model on roughly 22{,}000 records from a public dataset, it hit an $R^{2}$ of 0.90 using just three input features. This proves you do not need an overwhelming amount of data points to get a highly accurate burn rate prediction. On the front end, the employee portal gives workers immediate control over their data using visual gauges, history charts, and quick links to support resources. Meanwhile, the admin portal gives management a bird's-eye view of the workplace through department heatmaps, risk distribution graphs, KPI correlations, and automated warnings.

Architecturally, breaking the system into microservices worked well. Keeping the heavy ML logic in Python/FastAPI separated from the main web application backend built on Node.js/Express meant both sides could be built and deployed without breaking each other. Finally, the end-to-end functional testing proved the core workflows are fully operational. Everything works smoothly from the moment a user submits a quiz to the final dashboard calculations and automated alert triggers.

\subsection{Contributions}

This project delivers a few key contributions to the field. First, it introduces an integrated burnout detection platform. By combining psychometrics, machine learning, and organizational analytics inside a single web setup, it achieves a mix that none of the reviewed related works managed to do. Second, it maps out a practical CBI-to-ML pipeline. The system takes raw answers from the 13-question Copenhagen Burnout Inventory and converts them into a continuous burn rate prediction using a documented feature engineering workflow that handles reverse scoring, section averaging, and data normalization.

Third, it provides a working microservice pattern for developers looking to inject machine learning into web apps, complete with a clean toggle to switch between mock and real data for independent testing. Fourth, it provides a deep admin analytics suite that gives managers actionable tools like department heatmaps, risk graphs, KPI charts, and three distinct types of automated alerts for high risk, critical risk, and sudden trend spikes. Fifth, the project includes a complete design system and custom component library featuring 15 reusable UI primitives and 12 visualization components, including a custom SVG burn rate gauge built from scratch.

\subsection{Achievement of Objectives}

Every single one of the six objectives outlined back in Section~1.4 was completed. The CBI assessment works via a two-step wizard that properly calculates reverse scoring and derives the final mental fatigue metrics. Performance-wise, the Random Forest model hit an $R^{2} = 0.90$, an MAE = 0.048, and an RMSE = 0.061, which completely blew past the initial Linear Regression baseline. The employee portal is fully built out across four functional pages covering the dashboard, assessment wizard, results page, and history logs.For the managers, the admin portal delivers six functional pages including the main command center, department details with split tabs for employee lists and analytics, a company-wide directory, individual employee profiles, an alerts inbox, and a resource manager. The automated alert system successfully tracks and flags high risk, critical risk, and trend spikes, sorting them into a filterable inbox where admins can mark items as read. Lastly, the three-service architecture using React/Next.js, Node.js/Express, and Python/FastAPI is fully up and running with a functional mock/real mode switch.

\subsection{Limitations}
\label{sec:limitations}

Even though the system works, there are several limitations to keep in mind. A big one is the simulated HR data. Things like job titles and resource allocations were randomly generated when a user registered instead of pulling from an actual factory HR database. The values stay within realistic ranges based on the training data, but they do not map to a real workplace. Also, the feature set itself is quite narrow since the model relies on only three features. Adding variables like actual shift patterns, overtime logs, or long-term CBI trends would likely make the predictions much tighter. Relying on a single public Kaggle dataset for training and evaluation is another issue, meaning these results might not automatically hold up across different industries, factories, or cultures.

The KPI data suffers from a similar issue because metrics like attendance, productivity, and quality were seeded rather than gathered from real-world factory systems, which limits how deep we can read into the KPI-vs-burn-rate correlations. There is also a hardware limitation since the UI is strictly desktop-only right now, designed for screen widths of at least 992~px. This means floor workers using their personal smartphones cannot access it. Furthermore, the system lacks a formal usability study, so while the code works perfectly from a functional standpoint, we do not have structured feedback from actual human users yet. Finally, the admin dashboard fetches data using polling via TanStack Query rather than pushing real-time updates through WebSockets or Server-Sent Events, so managers have to wait for the next fetch cycle to see changes.

\subsection{Future Work}
\label{subsec:future}

Looking ahead, there are plenty of ways to expand this project. The first priority should be making the interface mobile responsive by adding flexible frontend breakpoints so factory workers can easily complete their wellness checks on their phones. Upgrading the communication layer to use WebSockets or Server-Sent Events would also be a massive improvement, allowing new alerts to pop up on the admin dashboard instantly without waiting on polling intervals. To make the system viable for actual deployment, replacing the simulated HR generator with direct API connectors to real platforms like SAP SuccessFactors or BambooHR would bring in authentic workload data.

On the data science side, it would be worth testing more advanced ML approaches like Gradient Boosting via XGBoost or LightGBM, alongside time-series models. This would let the system predict where an employee's burn rate is heading over time rather than just giving a snapshot of their current state. Expanding the feature engineering to pull in actual overtime hours, shift rotations, and seasonal patterns would also refine the model's accuracy. For better utility, adding quick CSV and PDF export buttons would let HR managers pull department reports and histories for offline meetings. The system also needs better accessibility, which means adding proper ARIA labels to custom visuals like the gauge or heatmap and running automated tests to match WCAG guidelines. Localization is another big step; translating the entire interface into languages like Vietnamese is essential to support the actual industrial workforces targeted by this tool. Lastly, setting up a formal usability study with real factory workers and HR staff will be crucial to figure out where the user experience falls short and how to polish it for the real world.
